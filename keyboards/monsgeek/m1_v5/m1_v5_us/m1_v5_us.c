...
void eeconfig_confinfo_default(void) {
    ...
    // eeconfig_init_user_datablock(); //depricated
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case RGB_MOD: {
            ...
            rgb_blink_dir();
            rgb_matrix_step();
            confinfo.record_last_mode = rgb_matrix_get_mode();
            // record_rgbmatrix_increase(&(confinfo.record_last_mode)); //depricated
            eeconfig_confinfo_update(confinfo.raw);
            start_hsv = rgb_matrix_get_hsv();
            }
            return false;
        ...
        case KC_LCMD: {
            if (keymap_is_mac_system()) {
                if (keymap_config.no_gui) { //removed  && !rgbrec_is_started() from if statement
                    if (record->event.pressed) {
        ...
        case HS_BATQ: {
            if ((confinfo.devs != DEVS_USB) && record->event.pressed) {
                md_send_devctrl(MD_SND_CMD_DEVCTRL_INQVOL);
            }
            return false;

            //This function was changed from
            //case HS_BATQ: {
            //     extern bool rk_bat_req_flag;
            //     rk_bat_req_flag = (confinfo.devs != DEVS_USB) && record->event.pressed;
            //     return false;
            // } break;
            //This is to preserve the function of the battery query after stripping the rgb recording stuff
            //As stated in the same place where the rgb recording stuff is, I'll fix it, but for now I'm keeping it out for simplicity's sake
        } break;
        case HS_DIR: {
            if (record->event.pressed) {
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    ...
    if (enable_bat_indicators && !inqbat_flag) { //removed  && !rgbrec_is_started() from if statement
        rgb_matrix_hs_bat();
        bat_indicators();
